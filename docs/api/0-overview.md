This describes the resources that make up the official Hello World API v1.
...
```
Accept: application/vnd.helloworld+json; version=1.0
```
...
Requests that return multiple items will be paginated to 30 items by default. You can specify page number with `?page` parameter. For some resources, you can also set a custom page size up to 1000 with the `?per_page` parameter.

Note that page is 1-based and that omitting the `?page` parameter will return results for page 1.
